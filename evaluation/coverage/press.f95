...
contains
      ...
      use common_sn ! create_new_include_statements() line 102
        ...
        real(kind=4) :: pav   ! Gav added
        real(kind=4) :: rhsav ! Gav added
        real(kind=4) :: area  ! Gav added
        real(kind=4) :: pco   ! Gav added
        real(kind=4) :: reltmp! Gav added
        real(kind=4) :: sor   ! Gav added
        ...
        real(kind=4), dimension(0:ip+2,0:jp+2,0:kp+1) :: p2 ! WV20160309 scratch pad for SOR 
        ! WV20160309 If your compiler is smart enough this should work with a 4-d array which would be used for the SOR calculation
        ! real(kind=4), dimension(0:ip+2,0:jp+2,0:kp+1,0:1) :: p2
...
!      
 ! WV20160309 set p2
      p2 = p
!     p2(0)=p ! for the 4-D case
!     p2(1)=p ! for the 4-D case	
...
! --SOR
      do l = 1,nmaxp
        ...
        do k = 1,km
        do j = 1,jm
        do i = 1,im ! WV20160309 was +mod(k+j+nrd,2),im,2
! WV20160309 if your compiler is smart enough the if-then could be replaced by indexing into the 4-d p2 array:
!            reltmp = omega*(cn1(i,j,k) *(cn2l(i)*p2(i+1,j,k,1-nrd) +cn2s(i)*p2(i-1,j,k,1-nrd) +cn3l(j)*p2(i,j+1, &
!        k,1-nrd) +cn3s(j)*p2(i,j-1,k,1-nrd) +cn4l(k)*p2(i,j,k+1,1-nrd) +cn4s(k)*p2(i,j,k-1,1-nrd) -rhs(i,j,k))-p2(i,j,k,1-nrd))
!            p2(i,j,k,nrd) = p2(i,j,k,1-nrd) +reltmp
          if (nrd==0) then
        ...
        k) +cn3s(j)*p(i,j-1,k) +cn4l(k)*p(i,j,k+1) +cn4s(k)*p(i,j,k-1) -rhs(i,j,k))-p(i,j,k))
            p2(i,j,k) = p(i,j,k) +reltmp
          else if (nrd==1) then
            reltmp = omega*(cn1(i,j,k) *(cn2l(i)*p2(i+1,j,k) +cn2s(i)*p2(i-1,j,k) +cn3l(j)*p2(i,j+1, &
        k) +cn3s(j)*p2(i,j-1,k) +cn4l(k)*p2(i,j,k+1) +cn4s(k)*p2(i,j,k-1) -rhs(i,j,k))-p2(i,j,k))
            p(i,j,k) = p2(i,j,k) +reltmp
          end if
